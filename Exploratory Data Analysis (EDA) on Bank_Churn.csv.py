...
# We'll perform statistical tests to explore relationships between features and churn.
# 
...
for col in numerical_cols:
    ...
    if p_val < 0.05:
        print(f"{col} differs significantly between churned and non-churned customers (p < 0.05).")
    else:
...
# 
# i.) Churn Rate: Likely imbalanced, with fewer customers churning (e.g., ~20%       churn rate), suggesting a need for techniques like oversampling or class-       weighted models in predictive modeling.
# 
# ii.) Correlations: Features like Age, Balance, and IsActiveMember may show          stronger correlations with churn, while CreditScore and EstimatedSalary        may have weaker relationships.
# 
# iii.) VIF: Most features are expected to have low VIF (< 5), indicating minimal      multicollinearity, though NumOfProducts and Balance might show moderate        correlation.
# 
# iv.) Statistical Tests:<br>
#     --> Categorical variables like Geography, Gender, and IsActiveMember are           likely significant predictors of churn (p < 0.05 in Chi-Square tests).<br>
#     --> Numerical features like Age and Balance may differ significantly               between churned and non-churned groups (p < 0.05 in t-tests).<br>
#     
